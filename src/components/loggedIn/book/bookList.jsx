...
import {Alert,Modal, ModalHeader, ModalBody, ModalFooter,Button,UncontrolledAlert} from 'reactstrap'
...
this.props.createNewBook(data)
    ...
    render() { 
     ...
     const errorMessage=this.props.error ? <UncontrolledAlert color="danger" className="text-center">
      {this.props.error}
    </UncontrolledAlert>:null  
  const list=this.props.books
  let filteredData
  list.length ? (  filteredData = list.filter(book => {
    return (
      book.typeOfBooks.toLowerCase().indexOf(this.state.search.toLowerCase()) !== -1 
    )
  })) : (filteredData=[])
 ...
 </table>:<div className="w-100 text-center"><Alert className="text-center text-big text-dark" color="danger"> No book found !!</Alert></div>
...
<div className="input-group mb-2">
  ...
  <div className="input-group-prepend">
    ...
    <div className="container">
     ...
     {allTable}
         ...
         <Modal name="addNewBook" isOpen={this.state.modal} toggle={this.toggle}>
  ...
  {errorMessage}        
    ...
    <div className="col-11 mx-auto col-sm-6 my-2">
      <input name="numOfBooks" value={this.state.numOfBooks} required onChange={this.handleAllChanges} type="text" className="form-control" placeholder="Number of Books"/>
    </div>
      ...
      <Modal name="deleteBook" isOpen={this.state.modalDelete} toggle={this.toggleDelete}>
        <ModalHeader toggle={this.toggleDelete}>Are You Sure To Delete ?</ModalHeader>
       
       <div className="text-center">
    <Alert color="danger" >
      <p className="text-b">This can be danger when you book that is borrowes that can couse looses</p>
    </Alert>
       </div>
...
<Modal name="Edit" isOpen={this.state.modalEdit} toggle={this.toggleEdit}>
  ...
  {errorMessage}        
        <div className="row">
    <div className="col-11 mx-auto col-sm-6 my-2">
      <input name="editBookType" value={this.state.editBookType} required onChange={this.handleAllChanges} type="text" className="form-control" placeholder="Book Type"/>
    </div>
    <div className="col-11 mx-auto col-sm-6 my-2">
      <input name="editNumOfBooks" value={this.state.editNumOfBooks} required onChange={this.handleAllChanges} type="text" className="form-control" placeholder="Number of Books"/>
    </div>
        ...
        <ModalFooter>
          ...
          <Button color="danger" onClick={this.toggleEdit}><b>Cancel</b></Button>
        </ModalFooter>
        </form>
      </Modal>
...
const mapDispatchToProps=(dispatch) =>{
  return {
    ...
    editBook:(data,bookId)=>dispatch(bookActions.editBookData(data,bookId) ),
